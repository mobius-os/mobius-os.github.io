#!/usr/bin/env sh
# Shared-VPS deploy: the launcher runs as its own Compose project on the
# external edge-launch network, and the host's shared edge proxy serves
# mobius.you / mobius.page from the mobius.Caddyfile fragment this script
# installs (producer contract in the edge repo's README).
set -eu

cd "$(dirname "$0")"

edge_network="edge-launch"
...
"$edge_dir/edgectl" install launch mobius.Caddyfile
